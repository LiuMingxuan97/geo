...
def world_to_pixel(
    ...
    for iteration in range(max_iterations):
        # 计算当前像素 (u, v) 对应的地理坐标
        current_lat, current_lon = lat_lon_func(
            u, v, height, time_lines, eph_lines_info, qua_lines_info
        )

        # 计算 (u+1, v) 和 (u, v+1) 对应的地理坐标（数值偏导）
        lat_du, lon_du = lat_lon_func(
            u + 1, v, height, time_lines, eph_lines_info, qua_lines_info
        )
        lat_dv, lon_dv = lat_lon_func(
            u, v + 1, height, time_lines, eph_lines_info, qua_lines_info
        )
...
def lat_lon_func(
    u: int, v: int, height: float, line_file, pos_file, qua_file
) -> tuple[float, float]:
    line_num = u
