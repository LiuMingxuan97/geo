...
def get_time_value(pos_file_path:str, qua_list_path:list, timestamp):
    ...
    with open(pos_file_path, 'r') as pos_f:
        ...
        for line_info in lines_info:
            ...
            pos_x_list.append(float(dat[1]))
            pos_y_list.append(float(dat[2]))
            pos_z_list.append(float(dat[3]))
    with open(qua_list_path, 'r') as qua_f:
        ...
        for i in range(len(qua_lines_info)):
            ...
            if float(dat[0]) >= timestamp:
                ...
                t0 = float(dat[0])
                q0 = [ dat[2], dat[3], dat[4], dat[1]]
                dat = qua_lines_info[i].split(' ')
                t1 = float(dat[0])
                q1 = [dat[2], dat[3], dat[4], dat[1]]
                break
        # print(q0)
        # print(q1)
...
if __name__=='__main__':
    qua_file = 'XSD-10_PMS_230014018068_01_CCD-1.att'
    interp_x, interp_y, interp_z = get_time_value('XSD-10_PMS_230014018068_01_CCD-1.eph',qua_file, 781416468.5904029608)
